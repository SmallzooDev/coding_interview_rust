...
fn recursion<R: BufRead, W: Write>(n: usize, prefix: &str, io: &mut IO<R, W>) {
    io.put(prefix).put("\"재귀함수가 뭔가요?\"").nl();
    ...
    if n == 0 {
        ...
        let new_prefix = format!("{}____", prefix);
        recursion(n - 1, &new_prefix, io);
    }

    io.put(prefix).put("라고 답변하였지.").nl();
}
...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    io.put("어느 한 컴퓨터공학과 학생이 유명한 교수님을 찾아가 물었다.")
        .nl();
