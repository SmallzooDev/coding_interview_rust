...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [n, m, x] = io.get([0usize, 0usize, 0usize])?;
    let mut graph = vec![vec![]; n + 1];
    let mut rev_graph = vec![vec![]; n + 1];

    for _ in 0..m {
        let a: usize = io.get(0usize)?;
        let b: usize = io.get(0usize)?;
        let t: i64 = io.get(0i64)?;

        graph[a].push((b, t));
        rev_graph[b].push((a, t));
    }

    let dist_from = dijk(&graph, x, n);
    let dist_to = dijk(&rev_graph, x, n);
    let mut max_dist = 0i64;

    for i in 1..=n {
        if i != x {
            let total = dist_from[i] + dist_to[i];
            max_dist = max_dist.max(total);
        }
    }
...
fn dijk(graph: &[Vec<(usize, i64)>], start: usize, n: usize) -> Vec<i64> {
