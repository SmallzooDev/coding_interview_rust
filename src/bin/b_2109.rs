...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _i in 0..n {
        let [fee, day] = io.get([0usize; 2])?;
        lectures.push((fee, day));
    }

    lectures.sort_by_key(|&(_, day)| day);
    let mut heap: BinaryHeap<Reverse<usize>> = BinaryHeap::new();

    for (fee, day) in lectures {
        heap.push(Reverse(fee));
    ...
    let output: usize = heap.iter().map(|Reverse(fee)| fee).sum();
