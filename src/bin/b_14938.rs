...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [n, max_len, rc] = io.get([0usize, 0usize, 0usize])?;
    let mut items = vec![0usize; n + 1];
    let mut graph = vec![vec![]; n + 1];

    for i in 1..=n {
        let item = io.get(0usize)?;
        items[i] = item;
    }

    for _ in 0..rc {
        let [from, to, w] = io.get([0usize, 0usize, 0usize])?;
    ...
    let mut res: usize = 0;
    for i in 1..=n {
        let cnt = get_items(&graph, &items, i, n, max_len);
        res = res.max(cnt);
    }
...
fn get_items(
    ...
    let mut dists = vec![usize::MAX; node_count + 1];
    ...
    while let Some(Reverse((dist, node))) = pq.pop() {
        ...
        for &(next_node, weight) in &graph[node] {
            let next_dist = dists[node] + weight;
            if next_dist < dists[next_node] {
                dists[next_node] = next_dist;
                pq.push(Reverse((next_dist, next_node)));
            }
        }
    }

    (1..=node_count)
        .filter(|&i| dists[i] <= max_len)
        .map(|i| items[i])
        .sum()
}
