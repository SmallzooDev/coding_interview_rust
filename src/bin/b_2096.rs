...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _ in 0..n {
        let line = io.get(vec![0usize; 3])?;
        nums.push(line);
    }

    let mut prev_max = [nums[0][0], nums[0][1], nums[0][2]];
    let mut prev_min = [nums[0][0], nums[0][1], nums[0][2]];

    for i in 1..n {
        let curr_max = [
            nums[i][0] + prev_max[0].max(prev_max[1]),
            nums[i][1] + prev_max[0].max(prev_max[1]).max(prev_max[2]),
            nums[i][2] + prev_max[1].max(prev_max[2]),
        ];

        let curr_min = [
            nums[i][0] + prev_min[0].min(prev_min[1]),
            nums[i][1] + prev_min[0].min(prev_min[1]).min(prev_min[2]),
            nums[i][2] + prev_min[1].min(prev_min[2]),
        ];
    ...
    let max_result = prev_max[0].max(prev_max[1]).max(prev_max[2]);
    let min_result = prev_min[0].min(prev_min[1]).min(prev_min[2]);

    io.put(max_result).put(" ").put(min_result).nl();
