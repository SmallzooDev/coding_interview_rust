...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [v, e] = io.get([0usize, 0usize])?;
    let start: usize = io.get(0usize)?;
    let mut graph: Vec<Vec<(usize, i64)>> = vec![vec![]; v + 1];
    ...
    let mut dist = vec![i64::MAX; v + 1];
    let mut pq = BinaryHeap::new();

    dist[start] = 0;
    pq.push(Reverse((0i64, start)));

    while let Some(Reverse((cur, node))) = pq.pop() {
        if cur > dist[node] {
            continue;
        }
