...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [jewel_cnt, bag_cnt] = io.get([0usize; 2])?;
    let mut jewels = vec![];

    for _ in 0..jewel_cnt {
        let jewel = io.get(vec![0usize; 2])?;
        jewels.push((jewel[0], jewel[1]));
    }

    let mut bags = vec![];
    for _ in 0..bag_cnt {
        let capacity = io.get(0usize)?;
        bags.push(capacity);
    }

    jewels.sort_by_key(|&(weight, _)| weight);
    bags.sort();

    let mut total_value = 0;
    let mut jewel_idx = 0;
    let mut available_jewels = BinaryHeap::new();

    for bag_capacity in bags {
        while jewel_idx < jewels.len() && jewels[jewel_idx].0 <= bag_capacity {
            let (_, price) = jewels[jewel_idx];
            available_jewels.push(price);
            jewel_idx += 1;
        }

        if let Some(max_price) = available_jewels.pop() {
            total_value += max_price;
        }
    }

    io.put(total_value as i64).nl();
    None
}
