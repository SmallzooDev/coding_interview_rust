...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    let mut cards = std::collections::VecDeque::new();

    for i in (1..=n).rev() {
        cards.push_front(i);

        for _ in 0..i {
            if let Some(back_card) = cards.pop_back() {
                cards.push_front(back_card);
            }
        }
    }

    let result: Vec<String> = cards.iter().map(|x| x.to_string()).collect();
    io.put(result.join(" ")).nl();
