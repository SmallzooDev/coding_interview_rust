...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [p3, p4, p0] = io.get([0usize, 0usize, 0usize])?;

    let total = p3 + p4 + p0;

    for t3 in 0..=total / 3 {
        let used_for_3 = t3 * 3;
        let remaining = total - used_for_3;
        ...
        if remaining % 4 == 0 {
            ...
            if need3 > available3 || need4 > available4 {
                continue;
            }

            let remaining_slots3 = available3 - need3;
            let remaining_slots4 = available4 - need4;

            if remaining_slots3 + remaining_slots4 == flexible {
                io.put(t3).put(" ").put(t4).nl();
                return None;
            }
        }
    }
