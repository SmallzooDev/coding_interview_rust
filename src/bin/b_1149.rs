...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    let mut dp = vec![vec![0usize; 3]; n];

    for i in 0..3 {
        dp[0][i] = io.get(0usize)?;
    }

    for i in 1..n {
        let cost = io.get(vec![0usize; 3])?;
        dp[i][0] = cost[0] + dp[i - 1][1].min(dp[i - 1][2]);
        dp[i][1] = cost[1] + dp[i - 1][0].min(dp[i - 1][2]);
        dp[i][2] = cost[2] + dp[i - 1][1].min(dp[i - 1][0]);
    }

    let result = dp[n - 1][0].min(dp[n - 1][1]).min(dp[n - 1][2]);
    io.put(result).nl();
    None
}
