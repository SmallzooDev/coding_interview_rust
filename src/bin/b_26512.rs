...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    loop {
        let [x, y]: [i64; 2] = io.get([0i64; 2])?;

        if x == 0 && y == 0 {
            break;
        }

        let x_bin = to_8bit_binary(x);
        let y_bin = to_8bit_binary(y);
        let neg_x_bin = to_8bit_twos_complement(-x);
        let neg_y_bin = to_8bit_twos_complement(-y);
        let diff_bin = to_8bit_signed_binary(x - y);

        io.put(format!("{} = {}", x, x_bin)).nl();
        io.put(format!("{} = {}", y, y_bin)).nl();
        io.put(format!("-{} = {}", x, neg_x_bin)).nl();
        io.put(format!("-{} = {}", y, neg_y_bin)).nl();
        io.put(format!("{} = {}", x - y, diff_bin)).nl();
        io.nl();
    }
...
fn to_8bit_binary(n: i64) -> String {
    format!("{:08b}", n as u8)
}

fn to_8bit_twos_complement(n: i64) -> String {
    format!("{:08b}", n as i8 as u8)
}

fn to_8bit_signed_binary(n: i64) -> String {
    if n >= 0 {
        format!("{:08b}", n as u8)
    } else {
        format!("{:08b}", n as i8 as u8)
