...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _ in 0..n {
        let tmp = io.get(vec![0i64; n])?;
        papers.push(tmp);
    }

    let mut res = (0, 0, 0);
    get_papers(&papers, 0, n, 0, n, &mut res);
    io.put(res.2).nl().put(res.1).nl().put(res.0).nl();
...
fn get_papers(
    ...
    col_start: usize,
    col_end: usize,
    res: &mut (usize, usize, usize),
) {
    if let Some(uniform) = is_uniform(graph, row_start, row_end, col_start, col_end) {
        match uniform {
            1 => res.0 += 1,
            0 => res.1 += 1,
            -1 => res.2 += 1,
    ...
    let row_size = (row_end - row_start) / 3;
    let col_size = (col_end - col_start) / 3;

    for i in 0..3 {
        for j in 0..3 {
            let r_start = row_start + i * row_size;
            let r_end = row_start + (i + 1) * row_size;
            let c_start = col_start + j * col_size;
            let c_end = col_start + (j + 1) * col_size;
            get_papers(graph, r_start, r_end, c_start, c_end, res);
        }
    }
}
...
fn is_uniform(
    ...
    let first = graph[row_start][col_start];
