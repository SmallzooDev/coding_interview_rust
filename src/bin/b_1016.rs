...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [min_n, max_n] = io.get([0i64, 0i64])?;

    let range = (max_n - min_n + 1) as usize;
    let mut is_not_square_free = vec![false; range];

    let limit = ((max_n as f64).sqrt() + 1.0) as i64;
    ...
    for i in 2..=limit {
        ...
        let start_multiple = (min_n + square - 1) / square;
        let current = start_multiple * square;

        if current >= min_n {
            let mut idx = (current - min_n) as usize;
            let step = square as usize;

            while idx < range {
                is_not_square_free[idx] = true;
                idx += step;
            }
        }
    }
    ...
    let mut count: i64 = 0;
    for &is_marked in &is_not_square_free {
        if !is_marked {
            count += 1;
        }
    }
