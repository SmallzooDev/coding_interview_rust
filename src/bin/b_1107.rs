...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let target: usize = io.get(0usize)?;
    let n: usize = io.get(0usize)?;
    
    let mut broken = vec![false; 10];
    if n > 0 {
        let broken_buttons: Vec<usize> = io.get(vec![0; n])?;
    ...
    let mut min_press = (target as i32 - current as i32).abs() as usize;
    
    for channel in 0..=1000000 {
        if can_make_channel(channel, &broken) {
            let digit_press = count_digits(channel);
            let move_press = (target as i32 - channel as i32).abs() as usize;
            
            min_press = min_press.min(digit_press + move_press);
        }
    }
...
fn can_make_channel(channel: usize, broken: &[bool]) -> bool {
    let channel_str = channel.to_string();
    for ch in channel_str.chars() {
...
fn count_digits(channel: usize) -> usize {
    if channel == 0 {
        1
    } else {
        channel.to_string().len()
    }
}
