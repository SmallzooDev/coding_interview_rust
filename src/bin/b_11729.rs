...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    let total_moves: i64 = (1 << n) - 1;
    io.put(total_moves).nl();
...
fn hanoi<W: Write>(n: usize, from: i64, to: i64, aux: i64, io: &mut IO<impl BufRead, W>) {
    if n == 1 {
        io.put(from).put(" ").put(to).nl();
    } else {
        hanoi(n - 1, from, aux, to, io);
        io.put(from).put(" ").put(to).nl();
        hanoi(n - 1, aux, to, from, io);
    }
}
