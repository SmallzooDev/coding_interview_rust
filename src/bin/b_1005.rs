...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _ in 0..n {
        let [cnt, tech] = io.get([0usize, 0usize])?;
        let mut build_times = vec![0; cnt + 1];
        for i in 1..=cnt {
            build_times[i] = io.get(0usize)?;
        }
        let mut tech_graph: Vec<Vec<usize>> = vec![vec![]; cnt + 1];

        for _ in 0..tech {
            let [from, to] = io.get([0usize, 0usize])?;
            tech_graph[to].push(from);
        }

        let target = io.get(0usize)?;
        let res = get_time(&tech_graph, &build_times, target);
        io.put(res).nl();
    }
...
fn get_time(tech_graph: &[Vec<usize>], build_times: &[usize], target: usize) -> usize {
    let n = build_times.len() - 1;
    let mut memo = vec![None; n + 1];

    fn dfs(
        node: usize,
        tech_graph: &[Vec<usize>],
        build_times: &[usize],
        ...
        let mut max_req_time = 0;
        for &req in &tech_graph[node] {
            max_req_time = max_req_time.max(dfs(req, tech_graph, build_times, memo));
        }
        let result = max_req_time + build_times[node];
        memo[node] = Some(result);
        result
    }

    dfs(target, tech_graph, build_times, &mut memo)
}
