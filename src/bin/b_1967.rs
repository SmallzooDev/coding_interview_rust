...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _ in 0..n - 1 {
        ...
        graph[from].push((to, weight as i64));
        graph[to].push((from, weight as i64));
    }

    let mut visited = vec![false; n + 1];
    let mut distances = vec![0i64; n + 1];
    dfs(&graph, &mut visited, &mut distances, 1, 0);

    let farthest_node = distances
        .iter()
        .enumerate()
        .skip(1)
    ...
    let mut visited_from_leaf = vec![false; n + 1];
    let mut distances_from_leaf = vec![0i64; n + 1];
    dfs(&graph, &mut visited_from_leaf, &mut distances_from_leaf, farthest_node, 0);

    let max_distance = *distances_from_leaf.iter().max().unwrap();

    io.put(max_distance).nl();
    None
}
...
fn dfs(
    ...
    for &(node, edge_weight) in &graph[start] {
        ...
        dfs(graph, visited, distances, node, dist + edge_weight);
    }
}
