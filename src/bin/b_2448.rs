...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    let mut board = vec![vec![' '; 2 * n]; n];

    draw_triangle(&mut board, 0, n - 1, n);

    for row in board {
        let line: String = row.into_iter().collect::<String>();
        io.put(line.as_str()).nl();
    }

    Some(())
}

fn draw_triangle(board: &mut Vec<Vec<char>>, x: usize, y: usize, size: usize) {
    if size == 3 {
        board[x][y] = '*';
        board[x + 1][y - 1] = '*';
        board[x + 1][y + 1] = '*';
        ...
        let new_size = size / 2;
        draw_triangle(board, x, y, new_size);
        draw_triangle(board, x + new_size, y - new_size, new_size);
        draw_triangle(board, x + new_size, y + new_size, new_size);
    }
}
