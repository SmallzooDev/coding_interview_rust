...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [n, r, c] = io.get([0usize, 0usize, 0usize])?;

    let result = z_order(n, r, c);
    io.put(result).nl();

    None
}

fn z_order(n: usize, r: usize, c: usize) -> usize {
    if n == 0 {
        return 0;
    }

    let size = 1 << (n - 1);
    let half_size = size * size;

    match (r / size, c / size) {
        (0, 0) => z_order(n - 1, r, c),
        (0, 1) => half_size + z_order(n - 1, r, c - size),
        (1, 0) => 2 * half_size + z_order(n - 1, r - size, c),
        (1, 1) => 3 * half_size + z_order(n - 1, r - size, c - size),
        _ => unreachable!(),
    }
}
