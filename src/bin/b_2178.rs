...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [y, x] = io.get([0usize; 2])?;
    let grid: Vec<String> = io.get(vec![String::new(); y])?;
    ...
    io.put(bfs(&grid, y, x)).nl();
    None
}

fn bfs(grid: &[Vec<usize>], y: usize, x: usize) -> usize {
    let mut visited: Vec<Vec<i32>> = vec![vec![-1; x]; y];
    let moveable: [(i32, i32); 4] = [(1, 0), (0, 1), (-1, 0), (0, -1)];
    ...
    while !queue.is_empty() {
        ...
        for (my, mx) in moveable {
            ...
            {
                visited[ny as usize][nx as usize] = visited[current.0 as usize][current.1 as usize] + 1;
                queue.push_back((ny, nx));
            }
        }
    }

    visited[y - 1][x - 1] as usize
}
