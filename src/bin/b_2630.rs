...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    let grid = io.get(vec![vec![0; n]; n])?;

    let mut white = 0;
    let mut blue = 0;

    divide(&grid, 0, 0, n, &mut white, &mut blue);
...
fn divide(grid: &Vec<Vec<usize>>, x: usize, y: usize, size: usize, white: &mut usize, blue: &mut usize) {
    if is_same_color(grid, x, y, size) {
        if grid[x][y] == 0 {
            *white += 1;
        } else {
            *blue += 1;
    ...
    let half = size / 2;
    divide(grid, x, y, half, white, blue);
    divide(grid, x, y + half, half, white, blue);
    divide(grid, x + half, y, half, white, blue);
    divide(grid, x + half, y + half, half, white, blue);
}

fn is_same_color(grid: &Vec<Vec<usize>>, x: usize, y: usize, size: usize) -> bool {
    let color = grid[x][y];
    for i in x..x + size {
        for j in y..y + size {
            if grid[i][j] != color {
