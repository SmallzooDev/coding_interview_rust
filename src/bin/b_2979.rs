...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [a, b, c]: [usize; 3] = io.get([0usize; 3])?;
    let mut count: Vec<usize> = vec![0; 101];

    for i in 0..3 {
        let [in_time, out_time]: [usize; 2] = io.get([0usize; 2])?;
        for j in in_time..out_time {
            count[j] += 1;
        }
    }

    let result = count.iter().fold(0, |acc, car_count| {
        match car_count {
            3 => acc + (c * 3),
            2 => acc + (b * 2),
            1 => acc + a,
