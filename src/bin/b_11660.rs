...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for i in 0..n {
        table.push(io.get(vec![0i64; n])?);
    }

    let mut prefix = vec![vec![0i64; n + 1]; n + 1];

    for i in 1..=n {
        for j in 1..=n {
            prefix[i][j] =
                table[i - 1][j - 1] + prefix[i - 1][j] + prefix[i][j - 1] - prefix[i - 1][j - 1];
        }
    }

    for _ in 0..m {
        let [x1, y1, x2, y2] = io.get([0usize, 0usize, 0usize, 0usize])?;
