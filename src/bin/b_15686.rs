...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let [len, cnt] = io.get([0usize; 2])?;
    let grid: Vec<Vec<usize>> = io.get(vec![vec![0usize; len]; len])?;
    ...
    let min_distance = get_min_chicken_distance(&houses, &chickens, cnt);
    io.put(min_distance).nl();
...
fn calculate_chicken_distance(houses: &[(usize, usize)], selected_chickens: &[(usize, usize)]) -> usize {
    houses.iter()
        .map(|&house| {
            selected_chickens.iter()
                .map(|&chicken| manhattan_distance(house, chicken))
                .min()
                .unwrap_or(usize::MAX)
        })
        .sum()
}
fn get_min_chicken_distance(houses: &[(usize, usize)], chickens: &[(usize, usize)], m: usize) -> usize {
    let mut min_total = usize::MAX;

    generate_combinations(chickens, m, 0, &mut Vec::new(), &mut |selected| {
        let total_distance = calculate_chicken_distance(houses, selected);
        min_total = min_total.min(total_distance);
    });
    ...
    F: FnMut(&[(usize, usize)]),
{
    ...
    for i in start..chickens.len() {
        current.push(chickens[i]);
        generate_combinations(chickens, m, i + 1, current, callback);
        current.pop();
    }
}
