...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    while let Some(current) = deq.pop_front() {
        if current == k {
            io.put(dist[current]).nl();
            return None;
        }
        ...
        for (next_pos, cost) in moves {
            ...
            let next_dist = dist[current] + cost;
            if next_dist < dist[next_pos] {
                dist[next_pos] = next_dist;
                if cost == 0 {
                    deq.push_front(next_pos);
                } else {
                    deq.push_back(next_pos);
                }
            }
        }
    }
