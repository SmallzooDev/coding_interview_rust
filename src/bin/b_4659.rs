...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    loop {
        let password: String = io.get(String::new())?;
        if password == "end" {
            break;
        }
        ...
        if is_valid_password(&chars) {
            io.put(format!("<{}> is acceptable.", password)).nl();
        } else {
            io.put(format!("<{}> is not acceptable.", password)).nl();
        }
    }
...
fn is_valid_password(pw: &[char]) -> bool {
    ...
    let has_vowel = pw.iter().any(|&c| vowels.contains(&c));
    if !has_vowel {
        return false;
    }
