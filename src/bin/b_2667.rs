...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    let mut graph = vec![];
    let mut houses = vec![];
    let movable = vec![(1, 0), (0, 1), (-1, 0), (0, -1)];

    for _ in 0..n {
        let line: String = io.get(String::new())?;
    ...
    for i in 0..n {
        for j in 0..n {
            if graph[i][j] != 0 {
                let tmp = dfs(&mut graph, &movable, i, j, n);
                houses.push(tmp);
            }
        }
    }
...
fn dfs(graph: &mut [Vec<usize>], movable: &Vec<(i32, i32)>, y: usize, x: usize, n: usize) -> usize {
    graph[y][x] = 0;
    let mut count = 1;
    ...
    for (dy, dx) in movable {
        ...
        if ny >= 0 && ny < n as i32 && nx >= 0 && nx < n as i32 {
            ...
            let nx = nx as usize;
            if graph[ny][nx] != 0 {
                count += dfs(graph, movable, ny, nx, n);
            }
        }
    }
    count
