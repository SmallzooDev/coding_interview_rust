...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _ in 0..n {
        let [arrival, duration] = io.get([0usize; 2])?;
        cows.push((arrival, duration));
    }

    cows.sort_by_key(|&(arrival, _)| arrival);

    let mut current_time = 0;

    for (arrival, duration) in cows {
        current_time = current_time.max(arrival);
        current_time += duration;
    }

    io.put(current_time).nl();
    None
}
