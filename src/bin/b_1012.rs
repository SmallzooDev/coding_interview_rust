...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _ in 0..cases {
        let [max_x, max_y, target_cnt]: [usize; 3] = io.get([0usize; 3])?;

        let mut grid: Vec<Vec<usize>> = vec![vec![0; max_x]; max_y];
        let mut targets: Vec<(usize, usize)> = vec![];
        let mut visited: Vec<Vec<bool>> = vec![vec![false; max_x]; max_y];

        for _ in 0..target_cnt {
            let [target_x, target_y]: [usize; 2] = io.get([0usize; 2])?;
            grid[target_y][target_x] = 1;
            targets.push((target_y, target_x));
        }
        ...
        for (y, x) in targets {
            if !visited[y][x] && grid[y][x] == 1 {
                dfs(&grid, &mut visited, y, x, max_y, max_x);
                output += 1;
            }
        }
...
fn dfs(
    ...
    for (dy, dx) in directions {
        ...
        if ny >= 0 && nx >= 0 && ny < max_y as i32 && nx < max_x as i32 {
            let (nr, nc) = (ny as usize, nx as usize);
            if !visited[nr][nc] && grid[nr][nc] == 1 {
                dfs(grid, visited, nr, nc, max_y, max_x);
            }
        }
    }
}
