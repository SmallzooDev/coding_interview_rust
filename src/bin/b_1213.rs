...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    let line: Vec<char> = io.get_line()?.chars().collect();
    ...
    let odd_chars: Vec<char> = char_map
        .iter()
    ...
    if odd_chars.len() > 1 {
        io.put("I'm Sorry Hansoo").nl();
        return None;
    }

    let mut chars: Vec<char> = char_map.keys().cloned().collect();
    chars.sort();
    ...
    let result = format!("{}{}{}", left_half, middle, right_half);
    io.put(result).nl();
