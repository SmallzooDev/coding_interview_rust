...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    for _ in 0..cases {
        ...
        for _ in 0..2 {
            let tmp: Vec<usize> = io.get(vec![0usize; n])?;
            stickers.push(tmp);
        }

        if n == 1 {
            io.put(stickers[0][0].max(stickers[1][0])).nl();
            continue;
        }

        let mut dp = vec![vec![0usize; n]; 2];
        dp[0][0] = stickers[0][0];
        dp[1][0] = stickers[1][0];
        ...
        for i in 2..n {
            dp[0][i] = stickers[0][i] + dp[1][i - 1].max(dp[1][i - 2]);
            dp[1][i] = stickers[1][i] + dp[0][i - 1].max(dp[0][i - 2]);
        }

        let result = dp[0][n - 1].max(dp[1][n - 1]);
        io.put(result).nl();
    }
    None
}
