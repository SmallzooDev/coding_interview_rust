...
fn solve<R: BufRead, W: Write>(io: &mut IO<R, W>) -> Option<()> {
    ...
    let result = matrix_power(n - 1);
    io.put(result[0][0]).nl();
...
fn matrix_power(mut n: i64) -> Matrix {
    let mut base = [[1, 1], [1, 0]]; // 기본 피보나치 행렬
    let mut result = [[1, 0], [0, 1]]; // 단위행렬
